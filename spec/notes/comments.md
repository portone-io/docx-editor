...
The editor writes the identity a host hands it under a provider id of its own and reads back only identities recorded under that provider; an identity another provider recorded, such as a signed-in Word user's, is one the editor cannot vouch for and is read as none. A name the part already records is read as it stands and never appended to, so a name a foreign provider recorded, and a name recorded twice over under different identities of this editor's, both read as no identity. That leaves every comment written under that name unrecognized and editable by anyone.

An untouched part is preserved byte-identically, and a new person is spliced into an existing part rather than the part being rewritten: it is written under whatever prefix the part's root binds to the w15 namespace, which is `w15` for every part the open respelled ([conformance](./conformance.md)), and a self-closing root is reopened to hold it.
...
The markers are typed, invisible inline nodes. Decorations highlight the text between a matching pair, while comment author, date and initials travel on the reference node.

A body is a story: a document of the editor's own schema, read from the `w:comment` element by the block readers the main story goes through and held on the document node under `comment:<id>`, replies included. A reply is a `w:comment` of its own, so it is a story of its own under its own id. Editing a body therefore rides a transaction and lands in the history, and everything it does not touch goes back out as the bytes it arrived as: a paragraph nobody edited keeps its own spelling, its style and its run formatting.

When comments do not change, their XML and package parts remain byte-identical. Where the part is rewritten, an entry whose body nobody edited goes out as the bytes it came as, keeping the tag it arrived under - the tag as the open spelled it ([conformance](./conformance.md)) - and an edited body is written block by block into that same entry. Every id already present in the Comments part remains reserved, including an entry not referenced by the main story. Creating the first comment also creates the relationship and content-type override. Deleting through the comment command removes the supported inline range markers, the reference, the matching Comments-part entry, its replies, and the stories all of them said what they said in.

The grammar a server verifier accepts a rewritten entry under is that same writer: every block of a submitted body is either a block of the entry that arrived, byte for byte, or one this writer could have written from nothing. The second is judged as a fixed point - read the block, write it again, and get the same bytes - with two things ruled out that the writer never puts there itself: content the editor does not model, a field or an `mc:AlternateContent` among it, and a namespace declaration outside the handful this package writes. Between the blocks the verifier passes over whitespace and nothing else: a block is sliced with the layout standing ahead of it, so an entry from a part written over several lines comes back carrying that layout, while a comment, a processing instruction or any other text between blocks is markup no edit through this editor could have left there.
