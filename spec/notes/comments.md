...
The grammar a server verifier accepts a rewritten entry under is that same writer: every block of a submitted body is either a block of the entry that arrived, byte for byte, or one this writer could have written from nothing. The second is judged as a fixed point - read the block, write it again, and get the same bytes - with two things ruled out that the writer never puts there itself: content the editor does not model, a field or an `mc:AlternateContent` among it, and a namespace declaration outside the handful this package writes.
