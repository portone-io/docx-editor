...
The same pass runs over each header, footer, and footnotes part an edit rewrote, with the part as its scope: the stories one part holds share one set of identifiers in the order the part writes them, and a name the body or another part holds is not compared.
An entry the part writes as the bytes it arrived as - a footnote separator, a continuation notice - claims every name it holds ahead of the stories that are written again, since nothing rewrites those bytes and so no later claimant standing in them could yield.
