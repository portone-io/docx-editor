...
`CT_SectPr` and `CT_SectPrBase` describe the same content; the paragraph-level form adds nothing the body-level form lacks, so one reader answers for both.
...
`CT_SectPr` is a sequence: the header and footer references first, then the section's own content, then `w:sectPrChange`. The references are the only repeatable children, at most one per `w:type` value (`default`, `first`, `even`), so a reference is identified by the variant it names rather than by its element name alone. A reference that names no variant names `default`. The order the registry holds is in [Child order](./childOrder.md).
...
`w:orient` is not applied on top of `w:pgSz`: a producer writes the width and the height already swapped for a landscape section, so honouring both would turn the paper back upright. `w:cols` and the page-start rules `w:type` lays down are read into the model but not drawn.
