...
The `default` story applies to odd pages and to all pages when odd/even stories are disabled. `w:titlePg` selects the `first` story for the first page of every section it is written in, whatever place that page takes in the document, and `w:evenAndOddHeaders` in Settings enables the `even` story. When an enabled first or even variant is not declared in the section being drawn, its displayed story is blank rather than falling back to `default`.

The on-screen page number begins at `w:pgNumType/@w:start` when declared and at 1 otherwise, and replaces `PAGE`; the visual page count replaces `NUMPAGES`. Both simple and complex forms of these two fields are recognized and projected as decimal numbers. Each section applies its own start to the visual page number, but restarting the count at a section boundary is not reproduced, so the number a later section shows is its own start plus the page's place in the whole document. Variant selection is not read off that number: `w:titlePg` and the odd/even choice both follow the page's place within its own section, counted from that section's declared start, so every section's first page is drawn as a first page. Other `w:pgNumType` settings, including number formats and chapter numbering, remain preserved but are not reproduced. Their source XML and cached results remain untouched.
