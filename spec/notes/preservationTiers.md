...
The level rather than the element name is what the sub-tables are keyed by, because the same name means different things in different places. `w:sdt` is a content control a paragraph reader unwraps, a wrapper around one cell under `w:tr`, a wrapper around one row under `w:tbl`, and a container of blocks under `w:body` and inside a cell. Keys are Clark names (`{namespace}localName`) so that `m:oMath`, which is not WordprocessingML at all, sits in the same map.

Each of the two wrappers is read onto the node it stands around, so neither level needs a node of its own for it: what the control states rides on the cell or on the row, and the export puts the wrapper back there. A wrapper the reader cannot hang on one node still stands the table down: a control holding anything but the one cell or row, whether that is several of them or another control; a control around a cell or a row that only continues a vertical merge, since those are created fresh on export and no wrapper could be recreated for them; and a control whose own shape the reader does not write. [Content controls](./contentControls.md) gives the ground for each.

A block-level control is read as a container of the same block content the level around it takes, so it has a sub-table of its own: `sdtContent`. A control whose type the specification restrains to a single run - `w:text`, `w:picture`, `w:date`, `w:comboBox`, `w:dropDownList`, `w14:checkbox` - stays the block placeholder every such control used to be, since a container taking any run of blocks would let a single Enter break that restraint. Those restrained types stay placeholders; the one control that leaves the tier is the one whose `w:sdtContent` is absent or empty, which is read as a node of its own that holds nothing and draws nothing, since a container of blocks has no way to say "nothing". [Content controls](./contentControls.md) holds the rest.

At the `p` and `wrapper` levels a control with nothing inside it is read as a node of its own as well ([Content controls](./contentControls.md)). So of the `w:sdt` those two levels meet, the `inline` tier keeps only the shapes this editor does not write back, which is `docx/sdt`'s list: one carrying no `w:sdtPr`, and one whose content element carries an attribute, does not stand last, or stands under another prefix. The tier holds much else besides - the revision containers, `w:fldSimple`, the math elements, and every `p` child no row names - and a control holding nothing leaves no fidelity note behind, the way a `w:r` with no children leaves none.
