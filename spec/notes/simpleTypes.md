...
A reader that refuses a fraction drops a width the file plainly states, so this project reads one and rounds when it writes the value back, which is what keeps a rewritten table the width the producer gave it.
