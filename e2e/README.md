...
| `hangulComposition.spec.ts` | Hangul syllable assembly, batchim movement, deletion, locked content, and a composition written over a whole comment |
...
| `blockControls.spec.ts` | Backspace at the start of a block-level content control, Delete at the end of it, and the caret left where it stood |
...
| `notesEditing.spec.ts` | Typing, formatting, and undo inside a footnote and an endnote, hangul composition across a relayout, every gesture that writes in front of the number a note is drawn by and every press that reaches for that place, in both kinds of note, the number kept when a note's text is deleted, Escape and a press on the number returning the caret, and note insertion from the right-click menu and the shortcuts |
