...
| `notesEditing.spec.ts` | Typing, formatting and undo inside a footnote, hangul composition across a relayout, over the note's own number and over the whole of a note, the number kept when the note's text is deleted, Escape returning the caret, and footnote insertion from the right-click menu and from its shortcut |
