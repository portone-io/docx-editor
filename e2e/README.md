...
| `notesEditing.spec.ts` | Typing, formatting, and undo inside a footnote and an endnote, hangul composition across a relayout, every gesture that writes in front of the number a note is drawn by in both kinds of note, the number kept when a note's text is deleted, Escape and a press on the number returning the caret, and note insertion from the right-click menu and the shortcuts |
