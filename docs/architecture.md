...
The final section properties live on the document node as `sectPr`, so an edit to the paper the document is written on rides a transaction like any other and select-all + delete cannot take the section away with the blocks.
