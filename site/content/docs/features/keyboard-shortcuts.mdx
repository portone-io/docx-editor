...
The editor comes with built-in controls, selectable document zoom, undo and redo, keyboard
navigation, and IME composition, in a read-only, comment-only, or editing
[mode](/docs/features/modes).
[Zoom](/docs/editor-api/zoom) covers setting the zoom level from your own controls.
...
events first, so an application can take over a binding such as `Mod+K` without replacing the
editor.
