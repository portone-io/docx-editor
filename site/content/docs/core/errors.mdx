...
| `unsupported-content` | A preserved block copied so that it stands in two places, a paragraph in a list nothing defines, a namespace prefix already bound to another namespace, or a changed endnote or footnote separator, which the file cannot carry yet. |
