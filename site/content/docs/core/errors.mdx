...
| --- | --- |
| `malformed-xml` | A bookmark marker whose partner is gone, or a comment part with no root element once a comment has changed. |
...
| `unsupported-content` | A preserved block copied so that it stands in two places, a paragraph in a list nothing defines, or a namespace prefix already bound to another namespace. |
| `missing-content-types` | A part the writer adds (an image, a numbering part, a comment part) in a package with no content types part to declare it in. |
