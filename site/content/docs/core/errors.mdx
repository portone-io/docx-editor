...
| `duplicate-preserved-block` | `unsupported-content` | The `node` standing in two places, and the `story` it stands in or `null` for the body. |
...
A `story` is the side story the problem is about: its `kind` (`comment`, `footnote`, `endnote`,
`header` or `footer`) and the `id` it stands under, which is a note's number, a comment's id, or a
header or footer part's path. A `part` is `media`, `numbering`, `comments`, `commentsExtended`,
...
`pos` is where the problem stands: the block or marker itself, and for a footnote or an endnote the
first reference to that note in the body, since the note's own text stands nowhere in it. A caller
can select that position to show which note to revert. It is absent for a problem of the package,
for a header or a footer, and for a note nothing in the body refers to.
...
for a bug in the editor, and such an error carries no `problem`.
