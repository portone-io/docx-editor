...
Every reason but `unwritable-part-root` and `missing-content-types` carries a `story`: the side
story the content stands in, or `null` for the body. A `story` has a `kind` (`comment`, `footnote`,
`endnote`, `header` or `footer`) and an `id`: a note's number, a comment's id, or a header or
footer part's path. A `part` is `media`, `numbering`, `comments`, `commentsExtended`, `people`,
...
predict is thrown under the same codes with no `problem`, for a part root that binds a prefix the
writer needs, such as `w`, to another namespace (`unsupported-content`), for a plugin that wrote an
invalid node or attr, or for a bug in the editor.
