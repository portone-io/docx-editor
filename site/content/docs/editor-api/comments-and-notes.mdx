...
Each kind of note has its own four commands, so a caller reads what it puts in off the name it
calls.
...
It writes the body you hand it as it stands: the built-in note view keeps the mark a note's number
is drawn from when an edit inside the note takes it away, while these commands add none, so a body
that does not open with that mark opens in Word without the note's number.

`openFootnote` and `openEndnote` answer `false` only for an id no reference in the text names and
for a separator entry: opening a note is reading it, so it applies under every mode, and the note it
opens takes no typing where the body is shut.
They are what the built-in editor runs when a reader clicks a note's number, so a surface of your
own can put the caret in a note from a control of its own; clicking the number the open note itself
begins with runs the way back, which puts the caret just after the reference and scrolls there.

A transaction that deletes the last reference to a note deletes the note with it, so one undo brings
both back; one that copies a reference gives the copy a new id and a copy of the note.
