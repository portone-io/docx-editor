...
| Command | What it does |
| --- | --- |
| `insertFootnote` | Puts a footnote reference at the end of the selection, calling a new footnote that holds no text yet, and opens that footnote for editing. |
...
does not.
`insertFootnote` and `setFootnoteBody` do not apply under the `readOnly` and `comment` modes or
where the reference stands in locked content.
...
It writes the body you hand it as it stands: the built-in footnote view keeps the mark a footnote's
number is drawn from when an edit inside the note takes it away, while this command adds none, so a
body that does not open with that mark opens in Word without the footnote's number.

`openFootnote` answers `false` only for an id no reference in the text names and for a separator
entry: opening a footnote is reading it, so it applies under every mode, and the footnote it opens
takes no typing where the body is shut.
It is what the built-in editor runs when a reader clicks a footnote's number, so a surface of your
own can put the caret in a footnote from a control of its own.
