...
`reason` naming the content it is about, and the `pos` where the problem stands - for a footnote or
an endnote, the first reference to it in the body, which a panel can select.
[Checking before export](/docs/core/errors#checking-before-export) lists the reasons and the codes.
...
[Core API](/docs/core#what-a-document-loses) describes the fields and coverage.
