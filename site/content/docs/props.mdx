...
| `mode` | `DocxEditorMode` | required | What the editor is for. See [Mode](#mode). |
...
The kind, the `author` comments are written under, and `editableComments` are read on every render
and take effect on the open document.
