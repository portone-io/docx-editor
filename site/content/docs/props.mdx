...
Give each `id` a distinct `name`: a name shared by two ids, or already used by a Word user or an
unattributed comment in the file, resolves to no identity, and its comments become everyone's to
edit.
