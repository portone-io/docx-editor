...
React 18 and React 19 are both supported, and the package declares `react` as a peer dependency so
your application decides which one it runs.
Every release is checked against both.
...
The App Router also wants `@portone-io/docx-editor` in `transpilePackages` when it resolves to
