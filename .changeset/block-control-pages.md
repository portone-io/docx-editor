...
A paragraph inside a control may end a section, since a control holds what the body holds (§17.5.2.34). Such a paragraph's `w:sectPr` was written back untouched but drawn as nothing: the paper, the margins, the headers and the footers of everything up to it were read off the next section instead. The break is now read, so the section it closes gets its own paper, its own header and footer stories, and note numbering restarting there where the section says so. It is read at the paragraph carrying it, so the blocks the control holds after that paragraph belong to the section that follows; the next page still opens after the whole control, where Word parts the control in two. A `w:sectPr` written inside a table cell is preserved rather than read, which is what Word makes of one.
